...
const PATH_DOT_RADIUS_FACTOR = 0.15; // fraction of tile size

export function renderPathDots(
  ctx: CanvasRenderingContext2D,
  characters: Character[],
  offsetX: number,
  offsetY: number,
  ...
  const s = TILE_SIZE * zoom;
  const dotRadius = s * PATH_DOT_RADIUS_FACTOR;
  ...
  for (const ch of characters) {
    ...
    for (const step of ch.path) {
      const cx = offsetX + step.col * s + s / 2;
      const cy = offsetY + step.row * s + s / 2;
      ctx.beginPath();
...
export function renderFrame(
  ...
  // Path dots (below furniture/characters, on top of floor)
  if (showPathDots) {
    renderPathDots(ctx, characters, offsetX, offsetY, zoom);
  }
